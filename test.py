...
for item in items:
    ...
    temp.append(carid.strip())
    temp.append(name.strip())
    temp.append(detail.strip())
    temp.append(price.strip())
    temp.append(service.strip())
    temp.append(link)
    temp.append(now)
    print(temp)
    carlist.append(temp)
...
with open('carlist.csv', 'w', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['ID','Name','Detail','Price','Service','Link','CreatedDate'])
